...
public class SyncRequest
{
	 ...
	 *
	 * @param key
	 *            Key to add in request's header.
	 * @param value
	 *            Value to add in request's header. It could be {@code null}.
	 */
	public SyncRequest setHeader(CharSequence key, CharSequence value)
	{
		httpConnection.setRequestProperty(key.toString(),
	...
	public SyncResponse doPost(byte[] content)
	{
		...
		} catch (ProtocolException e)
		{
			throw new UnsupportedOperationException(
					"unsupported method POST. contact author with stacktrace",
					e);
		}
	}
	...
	public SyncResponse doPatch(byte[] content)
	{
		...
		{
			throw new UnsupportedOperationException(
					"unsupported method PATCH. contact author with stacktrace",
					e);
		}
	}
	...
	public SyncResponse doPut(byte[] content)
	{
		...
		{
			throw new UnsupportedOperationException(
					"unsupported method PUT. contact author with stacktrace",
					e);
		}
	}
	...
	public SyncResponse doDelete()
	{
		...
		{
			throw new UnsupportedOperationException(
					"unsupported method DELETE. contact author with stacktrace",
					e);
		}
	}
	...
	public SyncResponse doGet()
	{
		...
		{
			throw new UnsupportedOperationException(
					"unsupported method GET. contact author with stacktrace",
					e);
		}
	}
	 ...
	 *             fail to network connection or fail to read response.
	 */
	...
	protected SyncResponse makeResponse()
	{
		try
		{
			...
			// TODO: how about directly pass this steams to jackson for
			// performance
			if (code < 400)
			...
			// which one is better?
			/*
			 * val byteStream = new ByteArrayOutputStream(); BufferedInputStream
			 * body;
			 * 
			 * if (code < 400) body = new
			 * BufferedInputStream(httpConnection.getInputStream()); else body =
			 * new BufferedInputStream(httpConnection.getErrorStream());
			 * 
			 * int bytes; while ((bytes = body.read()) != -1) {
			 * byteStream.write(bytes); }
			 * 
			 * byteStream.close(); body.close(); return new SyncResponse(url,
			 * code, message, header, byteStream.toByteArray());
			 */
		} catch (IOException e)
		{
			// FIXME: custom exception
